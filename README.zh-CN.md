...
本仓库当前提供 **五个**组合动作。它们均**不执行 `checkout`**——假定调用方工作流
已完成代码检出。
...
| [`docker-build-push`](.github/actions/docker-build-push/action.yml) | 构建并推送多架构 Docker 镜像（含缓存） | `docker/setup-qemu-action`、`docker/setup-buildx-action`、`docker/login-action`（可选）、`docker/metadata-action`、`docker/build-push-action` |
...
所有动作均为 `using: composite`，并封装已有的上游动作。在非 GitHub Runner（如 Gitea）
上使用时，必须允许从 GitHub 解析这些上游动作——见[平台说明](#平台说明)。
...
使用 Buildx 构建 Docker 镜像（支持多架构）并推送到镜像仓库。支持仓库登录、OCI
元数据/标签、构建参数、构建密钥、指定构建阶段，以及 GHA/镜像仓库层缓存。当
`push` 为 `false` 时仅构建（适用于 PR 校验）。

**委托对象**

- `docker/setup-qemu-action@v3`（始终）—— 多架构模拟
- `docker/setup-buildx-action@v3`（始终）
- `docker/login-action@v3` —— 仅当 `username` 与 `password` 均非空时
- `docker/metadata-action@v5`（始终）—— 推导标签/标签元数据
- `docker/build-push-action@v6`（始终）—— 输出 `provenance` 与 `sbom`
...
| `platforms` | `linux/amd64` | 否 | 逗号分隔的目标平台（如 `linux/amd64,linux/arm64`） |
| `push` | `true` | 否 | 构建完成后推送到仓库 |
| `cache-from` | `type=gha` | 否 | 缓存导入来源 |
| `cache-to` | `type=gha,mode=max` | 否 | 缓存导出目标 |
| `target` | _(空)_ | 否 | Docker 构建目标阶段 |
| `secrets` | _(空)_ | 否 | 构建密钥（JSON 格式，如 `{"TOKEN":"xxx"}`，会作为环境变量导出供构建使用） |
...
- 标签还会由 `docker/metadata-action` 自动追加：短 `sha` 标签，以及默认分支上的
  `latest` 标签。
- `build-args` / `secrets` 必须为合法 JSON；空字符串或 `{}` 视为「无参数 / 无密钥」。
- 启用了 `provenance` 与 `sbom`（`provenance: mode=max`，`sbom: true`）。
