...
The repository currently ships **six** composite actions. None of them performs
...
| [`docker-build-push`](.github/actions/docker-build-push/action.yml) | Build & push Docker images with the **local `docker` CLI** (`docker build` + `docker push`) | none — calls `docker` directly |
| [`docker-buildx-push`](.github/actions/docker-buildx-push/action.yml) | Build & push Docker images via upstream **`docker/build-push-action`** | `docker/setup-docker-action`, `docker/setup-qemu-action`, `docker/setup-buildx-action`, `docker/login-action`, `docker/metadata-action`, `docker/build-push-action` |
...
Most actions wrap upstream published actions; [`docker-build-push`](#docker-build-push)
is the exception — it drives the local `docker` CLI directly and depends on no
external Action. When used on a non-GitHub runner (e.g. Gitea) the upstream-action
based composites must be resolvable from GitHub — see [Platform notes](#platform-notes).
...
## `docker-build-push` (local `docker` CLI)

Builds and pushes a Docker image by driving the **local `docker` CLI client**
directly — `docker build` and `docker push`. It depends on **no external GitHub
Action** and does **not** use `buildx`, so it runs on any runner that has a plain
`docker` CLI (self-hosted, Gitea, or fully offline runners, provided the base
images and registry are reachable). Because it uses the classic builder, it builds
for the **runner's native platform only** — there is no multi-arch emulation.

**How it works (no upstream actions, no buildx)**

- Validates `registry`, `image-name`, the Dockerfile, and the context directory.
- Logs in with `docker login --password-stdin` when `username`/`password` are set.
- Runs `docker build` once with one `--tag` per entry in `tags`, plus `--build-arg`,
  `--label`, `--target`, `--pull`, and `--no-cache` as applicable.
- When `push` is `true`, pushes every tag with `docker push` and captures the
  `sha256` digest from the push output (exposed as `image-digest`). When `push` is
  `false`, `image-digest` carries the local image ID instead.
...
| `pull` | `true` | no | Always attempt to pull a newer base image |
| `push` | `true` | no | Push to the registry after build |
| `no-cache` | `false` | no | Disable all cache layers |
| `target` | _(empty)_ | no | Docker build target stage |
| `labels` | _(empty)_ | no | Image labels as JSON, e.g. `{"org.opencontainers.image.title":"my-app"}` |

**Outputs**

| Name | Description |
| --- | --- |
| `image-digest` | `sha256` digest of the pushed image; the local image ID when `push` is `false` |
| `image-tags` | Comma-separated list of built/pushed image tags |

**Behavior notes**

- Requires only a plain `docker` CLI in `PATH` — **no `buildx` plugin, no QEMU, no
  buildx builder**.
- Builds for the **runner's native architecture** only. For multi-platform images
  use [`docker-buildx-push`](#docker-buildx-push-upstream-action-based) instead.
- `build-args` / `labels` must be valid JSON; an empty string or `{}` is treated as
  "none".
- `docker build` uses the daemon's native layer cache; there is no GHA/registry
  cache import/export (unlike the buildx variant).
...
From another repository: `uses: persiliao/actions/docker-build-push@main`.

## `docker-buildx-push` (upstream-action based)

An alternative Docker build/push action that delegates to the official
[`docker/build-push-action`](https://github.com/docker/build-push-action) and the
companion `docker/setup-*` actions, instead of calling the `docker` CLI directly.
Use this when you prefer the maintained upstream action over the local-CLI
[`docker-build-push`](#docker-build-push) variant.

**Delegates to**

- `docker/setup-docker-action@v5` (always)
- `docker/setup-qemu-action@v4` — only for multi-platform builds
- `docker/setup-buildx-action@v4` (always)
- `docker/login-action@v3` — only when `username` and `password` are both set
- `docker/metadata-action@v6` (always) — derives tags/labels
- `docker/build-push-action@v6` (always)

**Key inputs**

| Name | Default | Description |
| --- | --- | --- |
| `docker-version` | `latest` | Docker version installed via `docker/setup-docker-action` |
| `registry` | `docker.io` | Container registry URL |
| `username` / `password` | _(empty)_ | Registry credentials (login skipped if empty) |
| `image-name` | — | **required** — image name without registry prefix |
| `tags` | `latest` | Comma-separated tags; `latest` is also honored by `metadata-action` |
| `dockerfile` | `./Dockerfile` | Dockerfile path |
| `context` | `.` | Build context |
| `build-args` / `secrets` | _(empty)_ | JSON maps |
| `platforms` | `linux/amd64` | Target platforms (auto-enables QEMU + `docker-container` builder) |
| `pull` | `true` | Always pull newer base images |
| `push` | `true` | Push after build |
| `cache-scope` | _(empty)_ | GHA cache scope (auto-derived from branch/PR if empty) |
| `target` | _(empty)_ | Build target stage |
| `github-token` | _(empty)_ | GitHub token for GHCR verification |
...
| `image-labels` | Labels applied to the image |
